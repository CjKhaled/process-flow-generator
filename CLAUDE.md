...
- `render/` — stage 3 entire, and the hosted page: a laid-out diagram in, one self-contained HTML page out. Deterministic, no LLM.
  - `assets.py` — stage 3's **only** file reads: the vendored bpmn-js dist files, and the page shell. Everything is inlined rather than linked, because Live Server's root is wherever the reader points it and any relative path into `node_modules` is broken by a different choice. The bundle is the **modeler**, not the navigated viewer, for the page's session-only Edit mode — three times the size, and unavoidable, since the drag machinery lives in modules the viewer bundle does not carry and taking only those would need a bundler.
...
It also owns **Edit mode**, which is the page's alone: a modeler is live from construction
and bpmn-js has no read-only switch, so the off state is built here. A module passed via
`additionalModules` vetoes every mutating command at priority 2000 — above the 1000
`BpmnRules` registers at — and CSS hides the palette and context pad. Editing is
**session-only and never saved**, which is what keeps an edited diagram from ever
disagreeing with `graph.json`, and is why no export exists to re-add the `X` marker
`bpmn/decisions.py` strips. The questions panel is not recomputed after an edit; the page
